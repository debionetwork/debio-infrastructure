...
module "postgresql" {
  ...
  ip_configuration = {
    ipv4_enabled        = false
    require_ssl         = false
    private_network     = module.gke_network.network_self_link
    allocated_ip_range  = module.postgresql_private_service_access.google_compute_global_address_name
    authorized_networks = []
  }
  module_depends_on = [
    module.postgresql_private_service_access.peering_completed
  ]
